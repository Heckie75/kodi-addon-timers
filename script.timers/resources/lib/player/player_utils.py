...
def preview(addon: xbmcaddon.Addon, timerid: int, player: 'xbmc.Player') -> None:
    ...
    if timer.is_playing_media_timer():
        ...
        if is_script(timer.path):
            ...
            play_smart_playlist(player=player, path=timer.path,
                                playListId=TYPES.index(timer.media_type))
...
def play_smart_playlist(player: 'xbmc.Player', path: str, playListId: int) -> None:
